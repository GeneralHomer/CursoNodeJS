...
function Sacar(){
    inquirer.prompt([
        ...
        inquirer.prompt([
            ...
            const amount = answer['amount'];
            if (subtractAmount(accountName, amount)) {
                console.log(chalk.bgYellow.green(`Foi sacado o valor de R$: ${amount} da sua conta!`));
            }
            operation();
        }).catch(err => console.log(err));
    }).catch(err => console.log(err));
...
function subtractAmount(accountName, amount){
    const accountData = getAccount(accountName);
    if(!amount || amount <= 0 || amount > accountData.balance){
        console.log(chalk.bgRed.black('Ocorreu um erro, valor inválido ou saldo insuficiente'));
        return false;
    }
